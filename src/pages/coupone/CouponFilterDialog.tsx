...
const CouponFilterDialog = ({
  ...
  const [maxUsageLimit, setMaxUsageLimit] = useState<string>(
    values.maxUsageLimit != null ? String(values.maxUsageLimit) : "",
  );
  ...
  const handleApply = () => {
    const limitNum = maxUsageLimit.trim() !== "" ? parseInt(maxUsageLimit, 10) : undefined;
    onApply({
      isActive,
      startDate,
      expireDate,
      maxUsageLimit: limitNum !== undefined && !Number.isNaN(limitNum) ? limitNum : undefined,
    });
    onOpenChange(false);
  };
        ...
        <div className="custom-scrollbar min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-5 sm:px-6">
          <div className="space-y-2">
            ...
            >
              ...
              <option value="no">غير نشط</option>
            </select>
          </div>
