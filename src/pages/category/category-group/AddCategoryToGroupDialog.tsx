...
import {
  useFetchAvailableCategoriesCursor,
...
const AddCategoryToGroupDialog = ({
  open,
  onOpenChange,
  groupId,
  onSuccess,
  ...
  const { mutate: addCategories, isPending } = useAddCategoriesToGroup();

  // Flatten paginated categories and get baseUrl from first page
  ...
  const handleSubmit = () => {
    ...
    addCategories(
      { groupId, categoryIds: selectedCategoryIds },
      {
        onSuccess: () => {
          toast.success("تم إضافة الفئات إلى المجموعة بنجاح");
          onOpenChange(false);
          onSuccess?.();
        },
        onError: (error: any) => {
      ...
      <DialogContent className="text-right max-w-2xl max-h-[80vh] flex flex-col">
        <DialogHeader className="text-right">
          <DialogTitle className="text-right flex items-center gap-2">
            ...
            إضافة فئة إلى المجموعة
          </DialogTitle>
          <DialogDescription className="text-right">
            اختر الفئات التي تريد إضافتها إلى هذه المجموعة
          </DialogDescription>
        </DialogHeader>
...
export default AddCategoryToGroupDialog;
