...
import { Upload, Trash2, Loader2, Tag, Image as ImageIcon } from "lucide-react";
import { toast } from "sonner";
import {
  useUpdateDiscountImage,
  useDeleteDiscountImage,
...
const DiscountImageDialog = ({ open, onOpenChange, discountId }: Props) => {
  const { data: discount, refetch: refetchDiscount } = useFetchDiscount(
    discountId,
    open && !!discountId
  );
  ...
  const currentImageUrl = discount?.image;
  ...
  const handleUpload = () => {
    ...
    updateImage(
      { discountId, image: selectedFile },
      {
        onSuccess: () => {
          toast.success("تم تحديث صورة الخصم بنجاح");
        ...
        onError: (error: any) => {
          toast.error(
            error?.response?.data?.message || "حدث خطأ أثناء تحديث صورة الخصم"
          );
        },
      }
    );
  ...
  const handleDelete = () => {
    ...
    deleteImage(discountId, {
      onSuccess: () => {
        toast.success("تم حذف صورة الخصم بنجاح");
        refetchDiscount();
        onOpenChange(false);
      },
      onError: (error: any) => {
        toast.error(
          error?.response?.data?.message || "حدث خطأ أثناء حذف صورة الخصم"
        );
      },
    });
  };
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md" dir="rtl">
        <DialogHeader className="text-right">
          <DialogTitle className="text-right">إدارة صورة الخصم</DialogTitle>
...
export default DiscountImageDialog;
