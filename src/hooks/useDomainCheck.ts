...
import { useCheckDomainAvailability } from "@/api/wrappers/domain.wrappers";
import { isValidDomain } from "@/utils/helpers";

export type DomainType = "subdomain" | "custom";

export function inferDomainType(domain?: string | null): DomainType {
...
export function normalizeDomainValue(domain?: string | null): string {
  if (!domain) return "";
  const clean = domain
    .trim()
    .replace(/^https?:\/\//, "")
...
/**
 * Availability state for both domain paths.
 *
 * A custom domain is priced by Dynadot while a platform slug is only checked
 * against our own stores, so the two branches hit different endpoints. Every
 * edit resets the result, which is what forces a re-check before a save or a
...
export function useDomainCheck({
  ...
  const checkDomainAvailabilityMutation = useCheckDomainAvailability();
  ...
  const checkDomain = useCallback(() => {
    ...
    const slug = normalizePlatformSlug(domain);
    ...
    checkDomainAvailabilityMutation.mutate(slug, {
      onSuccess: (data: any) => {
        setIsCheckingDomain(false);
        setDomainChecked(true);
        const available = data?.isAvailable ?? data?.data?.isAvailable ?? false;
