...
const GroupDeleteModal = ({
  ...
  const { mutate: updateGroup, isPending: isHiding } = useUpdateGroup();
  ...
  const handleHide = () => {
    ...
    updateGroup(
      ...
      {
        onSuccess: () => {
          toast.success("تم إخفاء المجموعة — لن تظهر للمستخدمين");
          onOpenChange(false);
          onSuccess?.();
        },
        onError: (error: any) => {
  ...
  return (
    <Dialog open={!!group} onOpenChange={(open) => !busy && onOpenChange(open)}>
      <DialogContent
        dir="rtl"
      ...
      >
        ...
        <div className="space-y-3 text-center sm:space-y-4">
          ...
          <p className="mx-auto max-w-[34rem] text-xs leading-6 text-slate-400 dark:text-[#a4b1fa] sm:text-lg sm:leading-8">
            ...
            إخفاء المجموعة من خيار الإخفاء ولن تظهر المجموعة للمستخدمين
          </p>
        </div>
